...
async function main() {
    ...
    // Network configurations (Updated with new contract addresses)
    ...
    // Swap parameters
    const amountIn = ethers.utils.parseEther("100"); // 100 tokens
    const amountOutMin = ethers.utils.parseEther("95"); // Minimum 95 tokens output (5% slippage)
    const feeAmount = ethers.utils.parseEther("0.5"); // 0.1 ETH for fees
    ...
    try {
        ...
        if (ethBalance.lt(feeAmount)) {
            throw new Error(`❌ Insufficient ETH for fees. Required: 0.1, Available: ${ethers.utils.formatEther(ethBalance)}`);
        }
        ...
        console.log("2. 🌉 Bridge stablecoin to Holesky");
        console.log("3. 🔄 Swap Stablecoin -> TokenA on Holesky");
        ...
        const swapTx = await CrossChainRouter.crossChainSwap(
            ...
            options,                      // LayerZero options
            {
                ...
                gasLimit: 3000000,        // High gas limit for complex operation
                gasPrice: ethers.utils.parseUnits("25", "gwei")
            }
        );
        ...
        for (const log of receipt.logs) {
            try {
                ...
                if (parsedLog.name === "CrossChainSwapInitiated") {
                    ...
                    console.log(`   📉 Min Amount Out: ${ethers.utils.formatEther(parsedLog.args.amountOutMin)} TokenA`);
                }
            } catch (e) {
        ...
        console.log("🌉 Step 2: Stablecoin being bridged to Holesky via LayerZero");
