...
async function main() {
    ...
    const router = await ethers.getContractAt("IPayfundsRouter02", arbitrumRouter);
    const amountInA = ethers.utils.parseUnits("2000", 18);
    const amountInB = ethers.utils.parseUnits("2000", 18);
    const amountAMin = ethers.utils.parseUnits("2000", 18);
    const amountBMin = ethers.utils.parseUnits("2000", 18);
